...
**Compatibility:** Public fields are additive. Limit enforcement requires Pi extension hooks available in `@earendil-works/pi-coding-agent` 0.78.0, so the subagent package's peer floor moves to that tested version. Native nested tool usage is returned for newer Pi versions; older compatible Pi versions still receive the same usage in `details.results` and Archimedes bus events.
