# Bounded Subagent Execution on v2.1.0

**Date:** 2026-08-15
**Branch:** `rebuild/subagent-execution-limits-210`
**Base:** `v2.1.0` (`2ed26aa1d3301cc01a927d9409778bbd13df4798`)
**Status:** COMPLETED

## Goal

Add optional per-child execution limits and truthful partial-state preservation to current pi-archimedes without regressing v2.1.0 model/thinking management, child-session correlation, startup behavior, or render contracts.
...
- Keep native v2.1.0 `childSessionId`, model/thinking local overrides, agent management, top-level execute import, and TypeBox package.
...
Port behavior tests without replacing v2.1.0 production files. Retain native child-session and local thinking-override tests and add model-label coverage for single/parallel live/completed render paths.
...
- [x] Native v2.1.0 child session IDs and model/thinking validation remain intact.
