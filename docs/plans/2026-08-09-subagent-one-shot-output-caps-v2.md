...
Add cold one-shot child execution to the bounded v2.0.1 stack. Keep exactly one provider request, preserve agentic timeout behavior, and apply a caller-supplied provider-native output-token cap when the emitted provider payload exposes a supported shape.
